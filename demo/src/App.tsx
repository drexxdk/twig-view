    ...
    >
      ...
      >
        <header style={{ marginBottom: 20 }}>
          <p style={{ margin: 0, color: "#7dd3fc", letterSpacing: "0.08em", textTransform: "uppercase", fontSize: 12 }}>
            twig-view demo
          </p>
          <h1 style={{ margin: "8px 0 0", fontSize: "clamp(2rem, 4vw, 3rem)" }}>Accessible tree view</h1>
        </header>

        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginBottom: 20 }}>
          <button type="button" onClick={() => treeRef.current?.expandAll()}>Expand all</button>
          <button type="button" onClick={() => treeRef.current?.collapseAll()}>Collapse all</button>
          <button type="button" onClick={() => treeRef.current?.focus("routing")}>Focus routing</button>
        </div>
          ...
          renderToggle={({ expanded, size }) => (
            <span
              ...
              style={{
                ...
                placeItems: "center",
                borderRadius: 999,
                background: expanded ? "rgba(56, 189, 248, 0.22)" : "rgba(148, 163, 184, 0.14)",
